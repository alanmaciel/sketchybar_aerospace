...
**`front_app`** is a single global item (`associated_display_mask=0`, shown on every display identically) that renders one glyph per *window* currently open in the focused AeroSpace workspace — not just the frontmost app's name. `plugins/front_app.sh` queries `aerospace list-windows --workspace focused --format "%{app-name}"`, maps each app name to a glyph via `plugins/icon_map.sh`'s `__icon_map` function, and concatenates them into the item's `label` (rendered with the `sketchybar-app-font` cask, `label.font` set in `sketchybarrc`). Multiple windows of the same app each get their own icon (no de-duplication). If you touch this, note it re-derives everything from `aerospace` on every event rather than trusting `$INFO`/`$FOCUSED_WORKSPACE` — simpler and avoids drift between the two subscribed events (`front_app_switched`, `aerospace_workspace_change`).
